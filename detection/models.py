...
class DetectionRule(models.Model):
    ...
    Keeping rules as data (not hardcoded logic) makes it easy to tune thresholds
    from the Django admin without touching code.
    """
    ...
    # Rule-specific thresholds (only the relevant one is used per rule)
...
class RuleMatch(models.Model):
    ...
    transaction = models.ForeignKey(
        Transaction, on_delete=models.CASCADE, related_name="rule_matches"
    )
